// components/smtp/tabs/SmtpTab.tsx
'use client';
...
import { SmtpConfigurationAssistant } from './smtp-config-assist';
import { Combobox } from '@/components/ui/combobox';
import { SmtpProfileList } from './smtp-list';
...
export function SmtpTab() {
  ...
  const handleSendTestEmail = async () => {
    ...
    if (!testRecipient.trim()) {
      setTestError(tCommon('admin.smtp.smtp.error_recipient_required'));
      return;
    }

    if (!isValidEmail(testRecipient)) {
      setTestError(tCommon('admin.smtp.smtp.error_email_invalid'));
      return;
    }
    ...
    try {
      ...
      setTestText(tCommon('admin.smtp.smtp.test_default_text'));
    } catch {
      setTestError(tCommon('admin.smtp.smtp.error_send_failed'));
    }
  };
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <section className="bg-white dark:bg-[#0c1023] rounded-sm p-6 border border-gray-200 dark:border-white/5 shadow-sm dark:shadow-none">
        <div className="mb-6 flex items-center justify-between">
          <h1 className="text-xl font-semibold text-gray-900 dark:text-white">{tCommon('admin.smtp.smtp.profiles_title')}</h1>
          <Button
            variant="outline"
            size="sm"
            onClick={() => setShowAssistant(!showAssistant)}
          >
            {showAssistant ? tCommon('admin.smtp.smtp.close_assistant') : tCommon('admin.smtp.smtp.find_config_btn')}
          </Button>
        </div>

        {showAssistant && (
          <div className="mb-6">
            <SmtpConfigurationAssistant />
          </div>
        )}
        ...
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <Input
            ...
            placeholder={tCommon('admin.smtp.smtp.search_placeholder_list')}
          ...
          <div className="flex items-center gap-2">
            <Combobox
              ...
              className="bg-white dark:bg-gray-800/50   text-gray-900 dark:text-white mt-0 min-w-[150px]"
            />
            <Button
              type="button"
              variant="outline"
      ...
      <section className="bg-white dark:bg-[#0c1023] rounded-sm p-6 border border-gray-200 dark:border-white/5 shadow-sm dark:shadow-none">
        ...
        <Dialog open={isTestDialogOpen} onOpenChange={setIsTestDialogOpen}>
          <DialogContent className="sm:max-w-lg bg-white dark:bg-[#0c1023] border border-gray-200 dark:border-white/10">
            <DialogHeader>
              <DialogTitle className="text-gray-900 dark:text-white">{tCommon('admin.smtp.resend.send_test')}</DialogTitle>
              <DialogDescription className="text-gray-500 dark:text-gray-400">
                {tCommon('admin.smtp.smtp.test_dialog_desc')}
              </DialogDescription>
            </DialogHeader>

            <div className="space-y-4 py-4">
              <div>
                <Label htmlFor="testRecipient" className="text-gray-700 dark:text-gray-300">{tCommon('admin.smtp.smtp.recipient_label')}</Label>
                <Input
                  id="testRecipient"
                  value={testRecipient}
                  onChange={(e) => {
            ...
            <DialogFooter className="border-t border-gray-200 dark:border-white/10 pt-4 bg-white dark:bg-[#0c1023]">
              <div className="flex w-full items-center justify-end gap-2">
                ...
                >
                  {isSendingTestEmail ? tCommon('admin.smtp.smtp.sending') : tCommon('admin.smtp.smtp.send')}
                </Button>
              </div>
            </DialogFooter>
          </DialogContent>
        ...
        <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
          <AlertDialogContent className="bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800">
            <AlertDialogHeader>
              <AlertDialogTitle className="text-gray-900 dark:text-white">{tCommon('admin.smtp.smtp.delete_confirm_title')}</AlertDialogTitle>
              <AlertDialogDescription className="text-gray-600 dark:text-gray-400">
                {tCommon('admin.smtp.smtp.delete_confirm_desc')}
              </AlertDialogDescription>
            </AlertDialogHeader>
